...
exports.generateBSPDungeon = generateBSPDungeon;
const T_WALL = 0;
const T_FLOOR = 1;
const T_CORRIDOR = 4;
// ─── Tile height jitter ────────────────────────────────────────────────────
...
function set(grid, x, y, tile) { if (inBounds(grid, x, y))
    grid[y][x] = tile; }
...
function digLCorridor(grid, ax, ay, bx, by, cw, rng) {
    ...
    for (let dy = -hw; dy <= hw; dy++)
        for (let dx = -hw; dx <= hw; dx++) {
            ...
            if (t !== T_FLOOR)
                set(grid, bendX + dx, bendY + dy, T_CORRIDOR);
        }
}
function connectSiblings(node, grid, rng, cw) {
    ...
    if (lc && rc)
        digLCorridor(grid, lc.x, lc.y, rc.x, rc.y, cw, rng);
}
function finaliseTiles(grid) {
    for (let y = 0; y < grid.length; y++)
        for (let x = 0; x < grid[y].length; x++)
            if (grid[y][x] === T_CORRIDOR)
                grid[y][x] = T_FLOOR;
}
// ─── Tile height jitter map ────────────────────────────────────────────────
function buildTileHeightMap(grid, bossRoom, jitterRng) {
    const map = {};
...
function placeRocks(rooms, grid, rng, spawnX, spawnZ, rockDensity = 1) {
    const rocks = [];
    const varieties = [
    ...
    rooms.forEach((room, ri) => {
        ...
        while (placed < rockCount && attempts < 60) {
            ...
            if (grid[rz][rx] !== T_FLOOR)
                continue;
            if (rx === spawnX && rz === spawnZ)
                continue;
            if (rocks.some(r => Math.abs(r.gridX - rx) < 1 && Math.abs(r.gridZ - rz) < 1))
                continue;
...
// ─── Public API ────────────────────────────────────────────────────────────
function generateBSPDungeon({ placeId = "", seed = Date.now(), gridWidth = 32, gridHeight = 32, cellSize = 4, wallHeight = 15, corridorWidth = 3, name = "Procedural Dungeon", difficulty = 1, textures = null, rockDensity = 1, } = {}) {
