...
function buildPalisade(width, height, spacing, stakeHeight, stakeRadius, entryDir, exitDir, doorWidth) {
    ...
    for (let x = -halfW + halfS; x <= halfW; x += spacing)
        if (!inGap(x, 'north'))
            addStake(x, halfH - rowOffset);
    for (let x = -halfW + halfS; x <= halfW; x += spacing)
        if (!inGap(x, 'south'))
            addStake(x, -(halfH - rowOffset));
    for (let z = -halfH + spacing + halfS; z < halfH; z += spacing)
        if (!inGap(z, 'west'))
            addStake(-(halfW - rowOffset), z);
    for (let z = -halfH + spacing + halfS; z < halfH; z += spacing)
        if (!inGap(z, 'east'))
            addStake(halfW - rowOffset, z);
...
function placeItems({ type, count, clearance, halfW, halfH, grid, rng, scaleRange = [1.0, 1.0], yOffset = 0 }) {
    ...
    for (let i = 0; i < count; i++) {
        for (let a = 0; a < MAX_ATTEMPTS; a++) {
            ...
            if (clearance > 0 && !grid.isAreaFree(x, z, clearance))
                continue;
            ...
            if (clearance > 0)
                grid.markOccupied(x, z, clearance);
            break;
        }
    }
    return items;
...
// ─── Public API ───────────────────────────────────────────────────────────────
function generateArea({ name = 'village', width = 200, height = 200, cellSize = CELL_SIZE, seed = Date.now(), theme = 'japanese_village', placeId, areaType, totalSmallHouse = 0, totalMediumHouse = 0, totalBigHouse = 0, totalSmallTrees = 10, totalMediumTrees = 5, totalBigTrees = 2, totalLightPoles = 10, totalGrass = 1000, totalHerbs = 10, totalMushrooms = 10, palisadeSpacing = 2.5, palisadeStakeHeight = 12, palisadeStakeRadius = 1.1, palisadeMargin = 6, palisadeDoorWidth = 8, wallHeight = 0, difficulty = 1, textures = null, floor: floorOverride = null, walls: wallsOverride = null, ceiling: ceilOverride = null, entry = 'north', exit = 'south', } = {}) {
    ...
    return {
        ...
        palisade: isVillage
            ? {
                ...buildPalisade(width + palisadeMargin * 2, height + palisadeMargin * 2, palisadeSpacing, palisadeStakeHeight, palisadeStakeRadius, entry, exit, palisadeDoorWidth),
